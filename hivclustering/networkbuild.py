...
def build_a_network(extra_arguments = None):
    ...
    arguments.add_argument('--prior-identity', dest = 'prior_identity', choices = ['entity', 'sequence'], default = 'entity', help = 'How nodes are matched against the prior network (-P) when deciding which nodes are new, which were removed, which moved between clusters, and how much each cluster grew. "entity" (the default) matches on the entity (person) ID, i.e. the node ID up to the first "%s", so that re-sequencing a person already in the network is not reported as growth; "sequence" matches on the full sequence ID (the behavior before v1.9.10). The two are equivalent when node IDs carry no entity prefix.' % ENTITY_ID_SEPARATOR)
    ...
    if run_settings.auto_prof is not None or run_settings.auto_threshold:
        ...
        if run_settings.auto_prof is not None:
            ...
            # Print all threshold data with recommendation indicator
            for r in profile:
                is_recommended = ""
                if selected_threshold is not None and abs(r[0] - selected_threshold) < 1e-10:
                    is_recommended = "*"
                print ("%g\t%d\t%d\t%d\t%d\t%d\t%g\t%d\t%s" % (r[0], r[1], r[2], r[3], r[4], r[5], r[6], r[7], is_recommended))
